...
featuredpath = "date"
linktitle = ""
title = "【Golang + オニオンアーキテクチャー】DDD を意識して Web API を実装してみる"
type = "post"
...
加えて、本プロジェクトでは、DDD と オニオンアーキテクチャー を取り入れます。<br>
（内容はほぼオニオンアーキテクチャーになってしまいましたが…）

DDD については、「DDD を Golang とオニオンアーキテクチャーでやるなら、こんな感じかな？」という個人の見解レベルです。<br>
...
---
# 採用アーキテクチャ：オニオンアーキテクチャー
---

[参考記事内](https://www.slideshare.net/pospome/go-80591000) で紹介されているのは <u>オニオンアーキテクチャー</u> をベースに <br>
...
オニオンアーキテクチャー における各層の依存関係 について説明します。
...
一般的なオニオンアーキテクチャーだと、上から下に一方向に依存します。<br>
...
したがって、HTTP通信以外でも対応できるように、本プロジェクトでは `/handler/rest` というふうにディレクトリを切っています。<br>
...
DDD や オニオンアーキテクチャー が絡んできて、結構重い内容になってきたので、一旦ここで切ろうと思います。<br>
後日、続編記事を出したいと思います。
